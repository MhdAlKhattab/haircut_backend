...
class OrderController extends Controller
{
    protected function validator(array $data)
    {
        return Validator::make($data, [
            ...
            'amount_pay_type' => 'required|string|in:cash,online,both',
            'cash_amount' => 'numeric',
    ...
    public function addOrder(Request $request)
    {
        ...
        if($request['amount_pay_type'] == 'both' and (!$request['cash_amount'] or !$request['online_amount'])){
            return response()->json(['errors' => 'You must send both cash and online amount!'], 400);
        }
        ...
        if($request['amount_pay_type'] == 'both' and $amount_after_discount != ($request['cash_amount'] + $request['online_amount'])){
            return response()->json(['errors' => 'Must cash plus online amount equal total amount!'], 400);
        }

        if ($request['amount_pay_type'] == 'cash'){
            $order->cash_amount = $amount_after_discount;
        }else if ($request['amount_pay_type'] == 'online'){
            $order->online_amount = $amount_after_discount;
        }else{
    ...
    //             'services.*' => 'numeric|exists:services,id',
    //             'amount' => 'numeric',
    //             'amount_pay_type' => 'string|in:cash,online,both',
    //             'cash_amount' => 'numeric',
    //             'online_amount' => 'numeric',
    ...
    //         return response()->json(['errors' => 'You must send both cash and online amount!'], 400);
    //     }
    //     if($request['amount_pay_type'] == 'both' and $request['amount'] != ($request['cash_amount'] + $request['online_amount'])){
    //         return response()->json(['errors' => 'Must cash plus online amount equal total amount!'], 400);
    //     }
    ...
    //     if ($order->amount_pay_type == 'cash'){
    //         $order->cash_amount = $order->amount;
    //         $order->online_amount = 0;
    //     }else if ($order->amount_pay_type == 'online'){
    //         $order->online_amount = $order->amount;
    //         $order->cash_amount = 0;
    //     }else{
    //         $order->cash_amount = $request['cash_amount'];
    //         $order->online_amount = $request['online_amount'];
